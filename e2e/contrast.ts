 ...
 *    files the node under `incomplete`. The teaching callouts and verdict rows
 *    are translucent tints over the panel, so their text composites onto
 ...
 *    certificate serials and signatures are printed in full and the chain lays
 *    out in three columns, so at 380px much of it is clipped away. Content scrolled past the client box is not dimmed or partly

 ...
 *     nothing, and this lab is a state machine: tampering a link, revoking via
 *     CRL or OCSP, swapping the trust store and switching the signature suite
 *     each rewrite the chain rendering and its verdicts.
 *
 ...
 * plausible offender: it prints certificate serials and signatures in full and
 * lays the chain out in three columns.
 */
 ...
 *
 * This is a chain-validation state machine, so both verdict branches matter:
 * a chain that validates and a chain that does not are different renderings
 * with different palettes, and it is the failure branches — tampered link,
 * revoked cert, wrong trust store, compromised CA — that a first-paint-only
 * gate never reaches.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  ...
  await scan(page, `${theme} / chain validated`);

  // Select each link in the chain; each renders its own certificate detail.
  for (const link of ['root', 'intermediate', 'leaf']) {
    await page.locator(`[data-select="${link}"]`).click();
    await scan(page, `${theme} / inspecting ${link}`);
  }

  // Tamper a link, which should make validation fail at exactly that link.
  await page.locator('[data-tamper="leaf"]').click();
  await page.locator('#run-validation').click();
  await scan(page, `${theme} / leaf tampered`);

  await page.locator('[data-tamper="intermediate"]').click();
  await page.locator('#run-validation').click();
  await scan(page, `${theme} / intermediate tampered`);

  await page.locator('#reset-lab').click();
  await scan(page, `${theme} / reset`);

  // Revocation, both mechanisms.
  await page.locator('#toggle-crl').click();
  await page.locator('#run-validation').click();
  await scan(page, `${theme} / CRL revoked`);

  await page.locator('#toggle-ocsp').click();
  await page.locator('#run-validation').click();
  await scan(page, `${theme} / OCSP revoked`);

  await page.locator('#reset-lab').click();

  // Trust stores: the same chain judged by three different root sets.
  for (const ctx of ['browser', 'os', 'application']) {
    await page.locator(`[data-context="${ctx}"]`).click();
    await scan(page, `${theme} / trust store: ${ctx}`);
  }

  // CA compromise scenarios.
  for (const c of ['root', 'intermediate', 'none']) {
    await page.locator(`[data-compromise="${c}"]`).click();
    await scan(page, `${theme} / compromise: ${c}`);
  }

  // Certificate Transparency: submission, inclusion and consistency proofs,
  // plus the mis-issuance detection that is the point of the exhibit.
  await page.locator('#ct-submit').click();
  await expect(page.locator('#ct-proof')).toBeEnabled();
  await scan(page, `${theme} / CT submitted`);

  await page.locator('#ct-proof').click();
  await scan(page, `${theme} / CT inclusion proof`);

  // A consistency proof is between two log STATES, so the button stays disabled
  // until a second certificate is in the log — one submission can never enable
  // it. Submit again rather than clicking a control that cannot become ready.
  await page.locator('#ct-submit').click();
  await expect(page.locator('#ct-consistency')).toBeEnabled();
  await scan(page, `${theme} / CT second submission`);

  await page.locator('#ct-consistency').click();
  await scan(page, `${theme} / CT consistency proof`);

  await page.locator('#ct-misissue').click();
  await scan(page, `${theme} / CT mis-issuance detected`);

  // The post-quantum signature suites.
  for (const pq of ['classical', 'hybrid', 'mldsa']) {
    await page.locator(`[data-pq="${pq}"]`).click();
    await scan(page, `${theme} / signatures: ${pq}`);
  }
}
